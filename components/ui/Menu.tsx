  ...
  return (
    <div data-ui className={`fixed inset-0 z-40 ${open ? "" : "pointer-events-none"}`} aria-hidden={!open}>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-black/60">{site.email || site.name}</span>
          <div className="flex gap-2">
            <a href={site.github} target="_blank" rel="noreferrer" aria-label="GitHub" tabIndex={open ? 0 : -1} className="grid h-9 w-9 place-items-center rounded-full bg-black text-white transition-transform hover:scale-105">
            ...
            {site.linkedin && (
              <a href={site.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" tabIndex={open ? 0 : -1} className="grid h-9 w-9 place-items-center rounded-full bg-black text-white transition-transform hover:scale-105">
